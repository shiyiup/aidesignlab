...
export const HeroBg: React.FC = () => {
  ...
  const gridLines = Array.from({ length: 12 }, (_, i) => ({
    y: (i / 12) * height,
    offset: Math.sin(progress * 0.5 + i * 0.5) * 5,
  }));
    ...
    >
      ...
      <svg style={{ position: "absolute", top: 0, left: 0, width: "100%", height: "100%" }}>
        {particles.map((p, i) => (
          <circle
            ...
            cy={p.y}
            r={p.size}
            fill={i % 3 === 0 ? "rgba(124,58,237,0.7)" : i % 3 === 1 ? "rgba(6,182,212,0.7)" : "rgba(167,139,250,0.6)"}
            opacity={p.opacity}
          />
        ))}
      </svg>
      ...
      <div
        style={{
          position: "absolute",
          inset: 0,
          background: "radial-gradient(ellipse 100% 100% at 50% 50%, transparent 40%, rgba(9,9,14,0.6) 100%)",
        }}
      />
    </div>
  );
